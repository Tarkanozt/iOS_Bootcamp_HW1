...
class AlgoruthmManager: AlgorithmProtocol {
    ...
    func isPalindrome(_ s: String) -> Bool {
        let stringLength = s.count
            ...
            while position < stringLength / 2 {
                let startIndex = s.index(s.startIndex, offsetBy: position)
                let endIndex = s.index(s.endIndex, offsetBy: -position - 1)
    ...
    func isAnagram(_ s: String, _ t: String) -> Bool {
        return s.lowercaseString.characters.sort() == t.lowercaseString.characters.sort()
    }
